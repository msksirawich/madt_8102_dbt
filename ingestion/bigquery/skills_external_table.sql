-- BigQuery External Table DDL for Skills
-- This table reads from GCS with Hive-style partitioning (dt=YYYY-MM-DD)

CREATE OR REPLACE EXTERNAL TABLE `madt-8102-479812.madt8102_bronze.skills`
(
  skill_id STRING,
  name STRING,
  category STRING,
...
OPTIONS (
  format = 'PARQUET',
  uris = ['gs://madt8102_bronze/skills/*'],  -- Pattern: dt=YYYY-MM-DD/*.parquet
  hive_partition_uri_prefix = 'gs://madt8102_bronze/skills',
  require_hive_partition_filter = false
);

-- Notes:
-- 1. The 'dt' column is automatically populated from the Hive partition path
-- 2. Data location: gs://madt8102_bronze/skills/dt=YYYY-MM-DD/
...
-- Query all data
-- SELECT * FROM `madt-8102-479812.madt8102_bronze.skills`;
